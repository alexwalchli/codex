...
import MentionsInput from '../components/intellisense/mentionsinput'
import Mention from '../components/intellisense/mention'
...
export class Node extends Component {
  ...
  handleOnKeyDown (e) {
    const { createNode, deleteNode, focusNodeAbove, undo, redo,
            focusNodeBelow, parentId, id, demoteNode, promoteNode } = this.props
    e.stopPropagation()
  ...
  renderNodeWidgets () {
    ...
    if (widgets.length && !widgetDataUpdating) {
      ...
      return (
        <div className='view-mode-content' dangerouslySetInnerHTML={this.getFinalNodeContent(finalContent)} />
      )
    } else {
  ...
  onAddNoteClicked (e) {
    const {id, toggleNodeMenu, focusNode} = this.props
    e.stopPropagation()
    toggleNodeMenu(id)
    focusNode(id, true)
    this.setState({
  ...
  render () {
    const { parentId, childIds, id, focused, externalList, collapsed, visible, selected, completed, notes, positionInOrderedList,
            nodeInitialized, currentlySelectedBy, currentlySelectedById, auth, toggleNodeMenu, menuVisible, displayMode } = this.props
      ...
      <div className={bulletClasses}>
        { typeof parentId !== 'undefined'
          ? <div className={`depth ${currentlySelectedCss}`}>
            <div className='menu-btn' onClick={(e) => this.onBulletMenuBtnClicked(e)}><i className='icon dripicons-menu' /></div>
            { menuVisible
                ? <div className='bullet-menu' onMouseLeave={() => toggleNodeMenu(id)}>
                  <ul>
                    <li onClick={(e) => this.onAddNoteClicked(e)}><i className='icon dripicons-document' />Add note</li>
                    <li onClick={(e) => this.onCompleteBulletClicked(e)}><i className='icon dripicons-checkmark' />
                      { completed
                        ? 'Re-open'
                        : 'Complete' }
                    </li>
                    { childIds.length > 0
                      ...
                      : null}
                    <li onClick={(e) => this.onDeleteBulletClicked(e)}><i className='icon dripicons-cross' />Delete</li>
                  </ul>
                </div>
              : null}
            ...
            <div className='content' onMouseEnter={this.handleOnMouseEnter} onMouseLeave={this.handleOnMouseLeave} onPaste={this.handlePaste}>
              ...
              ? <ul className='external-data-children'>
                { externalList.map((item) => {
                  return <li> {item.title} <br /><small>{item.points} | {item.author} | {item.numberOfComments} comments</small></li>
                })}
              </ul>
              : null }
            </div>
